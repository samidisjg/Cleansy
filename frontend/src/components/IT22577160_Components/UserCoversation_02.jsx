    ...
    <div className='w-full p-3'>
      {
         !open && (
            <>
               <h1 className="text-center py-3 font-extrabold text-3xl underline">Let's Chat</h1>
               {/* All messages list */}
               {
...
const MessageList = ({data, index, setOpen, setCurrentChat, me, setUserData, userData, online, setActiveStatus }) => {
   ...
   return(
      <div className={`w-full flex p-3 px-3 cursor-pointer  border border-slate-200 dark:border-slate-700  shadow-md rounded-sm bg-white dark:bg-gray-800`} onClick={(e) => setActive(index) || handleClick(data._id) || setCurrentChat(data) || setUserData(user) || setActiveStatus(online)}>
         ...
         <div className="pl-3">
            ...
            <p className="text-base">{data.lastMessageId !== userData?._id ? "You:": userData?.username?.split(" ")[0] + ": "} {data?.lastMessage}</p>
         </div>
      </div>
   )
}
...
const UserInbox = ({ setOpen, newMessage, setNewMessage, sendMessageHandler, messages, adminId, userData, activeStatus }) => {
   return (
      <div className='w-full min-h-full flex flex-col justify-between shadow-md rounded-md bg-white dark:bg-gray-800'>
         {/* message header */}
